(function (global, undefined) {
    'use strict';
    var T_READ_WRITE = 'readwrite';
    var T_READ_ONLY = 'readonly';
    ...
    function createInstance(type) {
        switch (type) {
            ...
            case 'db':
                return formInstanceData('db', DB.create());
            case 'ls':
                return formInstanceData('ls', LS.create());
        }
        return null;
    }
    ...
    function createAutoInstance() {
        ...
        if (instance) {
            return formInstanceData('db', instance);
        }
        ...
        if (instance) {
            return formInstanceData('ls', instance);
        }
        return formInstanceData('db', LS.create());
    }
    ...
    var DB = KvKeeper.StorageDB = function (indexedDb) {
         ...
         */
        that.setItem = function (key, value, callback) {
            var req = getTransactionStore(T_READ_WRITE).put({key: key, value: value});
         ...
         */
        that.getItem = function (key, callback) {
            var req = getTransactionStore(T_READ_ONLY).get(key);
         ...
         */
        that.hasItem = function (key, callback) {
            var req = getTransactionStore(T_READ_ONLY).openCursor(key);
         ...
         */
        that.removeItem = function (key, callback) {
            var req = getTransactionStore(T_READ_WRITE).delete(key);
         ...
         */
        that.getKeys = function (callback) {
            var req = getTransactionStore(T_READ_ONLY).openCursor();
            var keys = [];
         ...
         */
        that.getLength = function (callback) {
            var req = getTransactionStore(T_READ_ONLY).count();
         ...
         */
        that.clear = function (callback) {
            var req = getTransactionStore(T_READ_WRITE).clear();
