...
methods.forEach(function (method) {
    KvKeeper[method] = function () {
        ...
        KvKeeper.getStorage('ls', function (err, storage) { // TODO: type = auto
            if (err) {
...
var DB = KvKeeper.StorageDB = function (indexedDb) {
    ...
    this.ensureReady = function (callback) {
        openDb(indexedDb, function (err, db) {
            that._db = db;
            supplyResult(callback, err, that);
        });
    };
     ...
     */
    this.setItem = function (key, value, callback) {
        var req = getTransactionStore().put({key: key, value: value});

        wrapDbRequest(req, supplyResult(callback));
    };
...
function openDb(indexedDb, callback) {
    ...
    wrapDbRequest(req, function (err, event) {
        supplyResult(callback, err, event.target.result);
    });
...
function supplyResult(callback, err, data) {
